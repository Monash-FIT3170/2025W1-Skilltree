...
export default function RecentEvents({ limit = 4 }: { limit?: number }) {
  const { data } = useSWR<RecentEventsPayload>(`/api/events/recent?limit=${limit}`, fetcher);
          ...
          {payload.events.map((ev) => (
            <Card key={ev.id} className="rounded-xl">
              <CardContent className="p-4 flex items-start justify-between">
                ...
                <Badge className={ev.mode === "ranked" ? "bg-red-100 text-red-700" : "bg-emerald-100 text-emerald-700"}>
